...
def apply_presplit_feature_processing(
    X,
    y,
    experiment,
    mi_k=15,
    ...
    if experiment == "E3_spearman":
        validate_spearman_selection(
            spearman_selection_rule,
            spearman_selection_value,
        )
    ...
    if hasattr(X, "select_dtypes"):
        ...
        X_prepared = transform_features(
            X,
            encoder,
            preserve_categorical_codes=True,
        )
        discrete_mask = feature_type_mask(feature_names)
        if X_prepared.shape[1] != len(feature_names):
            raise RuntimeError(
                "Presplit prepared features do not align with feature names."
            )
    else:
        encoder = None
        X_prepared = np.asarray(X, dtype=np.float32)
        feature_names = [
            f"feature_{index}" for index in range(X_prepared.shape[1])
        ]
    ...
    if experiment == "E3_spearman":
        ...
        correlation = np.asarray(
            spearmanr(X_prepared, y, axis=0).statistic[:-1, -1],
            dtype=float,
        )
        ...
        if spearman_selection_rule == "top_k":
            selected = ranking[
                :min(
                    int(spearman_selection_value),
                    X_prepared.shape[1],
                )
            ]
        else:
            selected = np.flatnonzero(
                absolute >= float(spearman_selection_value)
            )
        ...
        selected_payload = {
            "method": "spearman",
            "selection_rule": spearman_selection_rule,
            "selection_value": spearman_selection_value,
            "coefficients": correlation.tolist(),
            "absolute_coefficients": absolute.tolist(),
        ...
        dimensions = {
            ...
            "selected_dimension": int(len(selected)),
            "final_dimension": int(len(selected)),
        }
        artifacts = {
        ...
        return (
            X_prepared[:, selected].astype(np.float32),
            dimensions,
            artifacts,
    ...
    if experiment == "E1_sequential_mi_pca":
        ...
        X_final = pca.fit_transform(
            sequential_scaler.fit_transform(X_mi)
        )
        dimensions = {
            "prepared_dimension": int(X_prepared.shape[1]),
...
def _metrics(y_true, y_pred, y_proba, normal_class_idx):
    ...
    if normal_class_idx is not None:
        ...
        y_true_binary = (
            np.asarray(y_true) != normal_class_idx
        ).astype(int)
        y_pred_binary = (
            np.asarray(y_pred) != normal_class_idx
        ).astype(int)
...
def _latency(
    ...
    return {
        ...
        "cold_start_total_ms": (
            model_load_seconds * 1000 + cold_start_prediction_ms
        ),
        "warm_batch_prediction_seconds": batch_seconds,
        "warm_inference_ms_per_sample": (
            1000 * batch_seconds / len(X_test)
        ),
...
def run_presplit_ablation_experiment(
    X,
    y,
    class_names,
    experiment,
    ...
    X_final, dimensions, artifacts, preprocessing_seconds = (
        apply_presplit_feature_processing(
            X,
            y,
            experiment,
            mi_k,
    ...
    print(
        f"    train/test shapes: {X_train.shape} / {X_test.shape}",
        flush=True,
    )
    ...
    config = {
        ...
        "result_warning": (
            "Presplit feature-ablation protocol; "
            "not a leakage-free holdout estimate."
        ),
    ...
    if model_name in DL_MODELS:
        return run_dl_presplit_experiment(
            X_train,
            X_test,
            y_train,
            y_test,
    ...
    for fold_number, (train_idx, val_idx) in enumerate(
        ...
        print(
            f"[CV {fold_number}/{n_splits}] model training started",
            flush=True,
        )
    ...
    print(
        ...
        f"{balancing_seconds:.1f} seconds",
        flush=True,
    )
    ...
    print(
        f"[FINAL] model training finished in "
    ...
    if artifacts.get("selected_features") is not None:
        with open(
            os.path.join(save_dir, "selected_features.json"),
            "w",
        ) as handle:
            json.dump(
