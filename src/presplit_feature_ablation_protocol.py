...
EXPERIMENTS = ("E1_sequential_mi_pca", "E2_parallel_mi_pca")
...
def apply_presplit_feature_processing(X, y, experiment, mi_k=15, pca_variance=0.95, random_state=42):
    ...
    if experiment == "E3_spearman":
        raise ValueError("E3 SPEARMAN: MISSING DEFINITION")
    if experiment not in EXPERIMENTS:
        raise ValueError(f"Unsupported professor experiment: {experiment}")
    ...
    selector = fit_mi_selector(X_prepared, y, k=mi_k, random_state=random_state,
...
def run_presplit_ablation_experiment(X, y, class_names, experiment, model_name="HGB", mi_k=15,
    ...
    """Run the presplit ablation protocol for one classical model."""
    print("[1] Loading/preparing full dataset", flush=True)
    print("[2] Applying presplit feature processing", flush=True)
    X_final, dimensions, _, preprocessing_seconds = apply_presplit_feature_processing(
        X, y, experiment, mi_k, pca_variance, random_state
    )
    ...
    config = {"protocol": PROTOCOL, "result_warning": "Presplit feature-ablation protocol; not a leakage-free holdout estimate.",
              "experiment": experiment, "model": model_name, "random_state": random_state,
              "mi_k": mi_k, "pca_variance": pca_variance, "n_splits": n_splits,
              "k_neighbors": k_neighbors, "rus_cap": rus_cap}
